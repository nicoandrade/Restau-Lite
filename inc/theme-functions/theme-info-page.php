...
function quemalabs_getting_started_menu() {
	add_theme_page( esc_attr__( 'Theme Info', 'restau-lite' ), esc_attr__( 'Theme Info', 'restau-lite' ), 'manage_options', 'restau_lite_theme-info', 'quemalabs_getting_started_page' );
}
...
function quemalabs_getting_started_page() {
	if ( ! current_user_can( 'manage_options' ) )  {
		wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'restau-lite' ) );
	}
	...
	<div class="getting-started-header">
		<div class="header-wrap">
			...
			<div class="theme-content">
				...
				<h4><?php esc_html_e( 'Getting Started', 'restau-lite' ); ?></h4>
				<h2 class="theme-name"><?php echo esc_html( QL_THEME_NAME ); ?> <span class="ver"><?php echo 'v' . esc_html( QL_THEME_VERSION ); ?></span></h2>
				<p><?php echo sprintf( esc_html__( 'Thanks for using %s, we appriciate that you create with our products.', 'restau-lite' ), esc_html( QL_THEME_NAME ) ); ?></p>
				<p><?php esc_html_e( 'Check the content below to get started with our theme.', 'restau-lite' ); ?></p>
				</div>
				...
				<ul class="getting-started-menu">
					...
					?>
					<li><a href="?page=restau_lite_theme-info&amp;tab=docs" class="<?php echo ( $tab == 'docs' ) ? ' active' : ''; ?>"><i class="fa fa-file-text-o"></i> <?php esc_html_e( 'Documentation', 'restau-lite' ); ?></a></li>
					<li><a href="https://www.quemalabs.com/theme/restau/" target="_blank"><i class="fa fa-star-o"></i> <?php esc_html_e( 'PRO Version', 'restau-lite' ); ?></a></li>
					<li><a href="https://www.quemalabs.com/" target="_blank" class="<?php echo ( $tab == 'more-themes' ) ? ' active' : ''; ?>"><i class="fa fa-wordpress"></i> <?php esc_html_e( 'More Themes', 'restau-lite' ); ?></a></li>
				</ul>
	...
	if ( $pagenow == 'themes.php' && $_GET['page'] == 'restau_lite_theme-info' ){
		if ( isset ( $_GET['tab'] ) ){
			$tab = $_GET['tab'];
		}else{
			$tab = 'docs';
